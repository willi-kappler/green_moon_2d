...
async fn main() -> Result<(), GMError> {
    ...
    let mut emitter1 = GMParticleEmitter::new_from_resource(&resources, "particle1", 200.0, 300.0);
    emitter1.set_active(true);
    emitter1.set_speed_max(2.0);
    emitter1.set_duration(2.0);
    emitter1.set_rot_speed_max(0.1);

    let mut emitter2 = GMParticleEmitter::new_from_resource(&resources, "head1", 600.0, 300.0);
